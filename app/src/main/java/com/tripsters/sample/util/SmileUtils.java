...
public class SmileUtils {
    ...
    public static final String[] ee_ = new String[] {ee_1, ee_2, ee_3, ee_4, ee_5, ee_6, ee_7,
            ee_8, ee_9, ee_10, ee_11, ee_12, ee_13, ee_14, ee_15, ee_16, ee_17, ee_18, ee_19,
            ee_20, ee_21, ee_22, ee_23, ee_24, ee_25, ee_26, ee_27, ee_28, ee_29, ee_30, ee_31,
            ee_32, ee_33, ee_34, ee_35};

    private static final Factory spannableFactory = Factory.getInstance();

    private static final Map<Pattern, Integer> emoticons = new HashMap<Pattern, Integer>();
    private static final Map<String, Integer> emoticonReses = new HashMap<String, Integer>();
    ...
    private static void addPattern(Map<Pattern, Integer> map, String smile, int resource) {
        ...
        emoticonReses.put(smile, resource);
    }
    ...
    public static Spannable getExpressionFromField(Context context, String fieldname) {
        // 这里用的反射，所以混淆的时候不要混淆SmileUtils这个类
        try {
            Field field = SmileUtils.class.getField(fieldname);

            return getSmiledText(context, (String) field.get(null));
    ...
    public static ImageSpan getExpressionFromText(Context context, String expressionText) {
        int index = -1;

        for (int i = 0; i < ee_.length; i++) {
            if (ee_[i].equals(expressionText)) {
                index = i;
                break;
            }
        }
        ...
        if (index >= 0) {
            return getExpression(context, expressionText);
        }

        return null;
    }

    public static ImageSpan getExpression(Context context, String expressionText) {
        if (emoticonReses.containsKey(expressionText)) {
            return new ImageSpan(context, emoticonReses.get(expressionText));
        }

        return null;
    }

    public static List<String> getExpressionReslist() {
        List<String> reslist = new ArrayList<String>();

        for (int i = 1; i < ee_.length; i++) {
            reslist.add(getFilename(i));
        }
