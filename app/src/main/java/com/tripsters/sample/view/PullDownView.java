...
public class PullDownView extends FrameLayout implements OnGestureListener, AnimationListener {
    ...
    private void init() {
        UPDATE_LENGHT =
                getContext().getApplicationContext().getResources()
                        .getDimensionPixelSize(R.dimen.updatebar_height);
        ...
        initArrow();
        dropDownString =
                getContext().getApplicationContext().getResources().getString(R.string.drop_dowm);
        releaseUpdateString =
                getContext().getApplicationContext().getResources()
                        .getString(R.string.release_update);
        doingUpdateString =
                getContext().getApplicationContext().getResources()
                        .getString(R.string.doing_update);
    }

    private void initArrow() {
        mUpArrow = getResources().getDrawable(R.drawable.pull_refresh_arrow_up);
        mDownArrow = getResources().getDrawable(R.drawable.pull_refresh_arrow_down);
    }
    ...
    public static void onGMTChange() {
        DISPLAY_DATE_FORMAT = new SimpleDateFormat("MM-dd HH:mm", Locale.US);
    }

    // public void initSkin() {
    // // Drawable bg = Utils.getGlobalBackground(getContext().getApplicationContext());
    // // child.setBackgroundDrawable(bg);
    // // //sxx LGE612,LGP705 首次进入主信息流下拉刷新 出现重影 20120511[S]
    // // if(!android.os.Build.VERSION.SDK.equals("15")){//android 4.0
    // // child.setBackgroundDrawable(bg);
    // // mUpdateContent.setBackgroundDrawable(bg);
    // // mArrow.setBackgroundDrawable(bg);
    // // mTitle.setBackgroundDrawable(bg);
    // // mProgressBar.setBackgroundDrawable(bg);
    // // }
    // //
    // // sxx LGE612,LGP705 首次进入主信息流下拉刷新 出现重影 20120511[E]
    //
    ...
    // mTitle1.setTextColor(getResources().getColor(R.color.tb_middle_grey));
    // mTitle2.setTextColor(getResources().getColor(R.color.tb_middle_grey));
    // }
    ...
    protected void addUpdateBar() {
        mAnimationUp =
                AnimationUtils
                        .loadAnimation(getContext().getApplicationContext(), R.anim.rotate_up);
        ...
        vUpdateBar =
                LayoutInflater.from(getContext().getApplicationContext()).inflate(
                        R.layout.view_update_bar, null);
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        boolean isStart = contentView.getTop() == 0;
        if (contentView instanceof AdapterView<?>) {
            AdapterView<?> v = (AdapterView<?>) contentView;
            if (v.getCount() == 0 || v.getChildCount() == 0) {
                return false;
            }
            isStart = v.getFirstVisiblePosition() == 0;
            if (isStart) {
                isStart = v.getChildAt(0).getTop() == v.getPaddingTop();
            }
        } else if (contentView instanceof ScrollView) {
            ScrollView v = (ScrollView) contentView;
            if (v.getChildCount() == 0) {
                return false;
            }
            if (isStart) {
                isStart = v.getScrollY() == 0;
            }
        }
        // isStart用来判断contentView是不是滚动到最上位置，如果contentView在最上位置，继续下拉则响应下拉事件，否则将事件交给contentView自己处理
        ...
        switch (mState) {
            ...
            case STATE_OPEN_MAX_RELEASE:
                ...
                if (mIsShowDate && mState == STATE_OPEN_MAX_RELEASE) {
                    ...
                    mTitle2.setVisibility(VISIBLE);
                    mTitle2.setText(getContext().getApplicationContext().getString(
