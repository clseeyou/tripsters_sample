	...
	public static Drawable fromDrawable(Drawable drawable, float radius, int border, int borderColor, int coverBorder, int coverBorderColor) {
		if (drawable != null) {
			if (drawable instanceof TransitionDrawable) {
		    	...
		    	for (int i = 0; i < num; i++) {
		    		...
		    		if (d instanceof ColorDrawable) {
			    		...
			    		drawableList[i] = new RoundedDrawable(drawableToBitmap(td.getDrawable(i)), radius, border, borderColor, coverBorder, coverBorderColor);
		    		}
		    	}
			...
			if (bm != null) {
				...
				LogUtils.logw(TAG, "Failed to create bitmap from drawable!");
			}
		}
		return drawable;
	}
